...
import Event from "../Mygroup-games/Event";
...
const Allgamesingroup = (props) => {
  ...
  const [number, setnumber] = useState(0);
    ...
    API.getGamesInaGroup(params.id, localStorage.getItem("token")).then(
      (data) => {
        setGame(data.Games);
        setGroup(data);
      }
    ...
    API.findEventinaGroup(params.id, props.token).then((data) => {
  ...
  return (
    <div>
      <Link to={"/mygroup"} className="backtogroups">
        ⬅️ back to {props.userName}'s group
      </Link>
      <Timer groupName={group.name} groupId={group.id} number={number} ></Timer>
      <div>
        <div className="yourGroup">
          <Form.Text className="text-muted">Game Collection</Form.Text>
          <div>
